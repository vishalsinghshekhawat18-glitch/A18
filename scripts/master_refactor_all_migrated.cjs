...
function decomposeBlock(block) {
  ...
  for (let i = startIdx; i < lines.length; i++) {
    ...
    if (trimmed.startsWith('### ') || trimmed.startsWith('## ') || trimmed.match(/^[⚡📖🏛️🧮⚖️❓📌🗺️]\s+\d+\./)) {
      flushBullets();
      flushTable();
      flushParagraph();
      resultBlocks.push({
...
const allMigratedFiles = fs.readdirSync(corpusDir).filter(f => f.startsWith('migrated-core-') || f.startsWith('migrated-eng-') || f.startsWith('migrated-quant-') || f.startsWith('migrated-pyqs-'));
let refactoredFiles = 0;
let totalDecomposedBlocks = 0;

allMigratedFiles.forEach(file => {
...
console.log(`🎉 Master Refactoring Complete across ${refactoredFiles} chapters!`);
console.log(`✨ Generated ${totalDecomposedBlocks} new structured semantic blocks!`);
