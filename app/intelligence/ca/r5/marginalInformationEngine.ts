/**
 * R5.3 Marginal Information Value (MIV) & Knowledge Cohesion Engine
 * Solves the Relative Opportunity-Cost problem:
 * 1. Global Monthly Thematic Clustering
 * 2. Multi-Story Cohesion Scoring
 * 3. Marginal Information Value (MIV) Ranking
 * 4. High-Density Editorial Memory Unit Synthesis
 */
...
export interface KnowledgeCluster {
  ...
  primaryExamConcept: string;
  masterArticle: CandidateArticle;
  attachedArticles: CandidateArticle[];
  statutoryAnchor?: string;
  totalUniqueFacts: string[];
}

export interface MIVEvaluation {
  artId: string;
  page: number;
  title: string;
  clusterId: string;
  ...
  cohesionType: 'STRONG_COLOCATION' | 'MODERATE_COLOCATION' | 'WEAK_COLOCATION' | 'UNRELATED';
  absoluteExamRelevance: number;
  uniqueInfoContribution: number;
  overlapWithMaster: number;
  mivScore: number; // MIV = Absolute Relevance * Unique Contribution / (1 + Overlap)
  finalAction: 'CREATE_NEW_MEMORY_UNIT' | 'ATTACH_TO_EXISTING_MEMORY_UNIT' | 'SKIP_LOW_MIV';
  standaloneJustification: string;
}

export interface SynthesizedMemoryUnitV3 {
  unitId: string;
  title: string;
  category: string;
  ...
  examAngle: string;
  whyStandaloneJustification: string;
}

// Pre-defined Sovereign & Thematic Knowledge Anchor Regexes
const KNOWLEDGE_ANCHOR_PATTERNS: Array<{
  clusterId: string;
  clusterTitle: string;
  category: string;
  tier: 'TIER_A' | 'TIER_B_PLUS';
...
export function executeMarginalInformationClustering(articles: CandidateArticle[]): {
  evaluations: MIVEvaluation[];
  synthesizedUnits: SynthesizedMemoryUnitV3[];
  falseStandaloneIdentified: any[];
} {
  const clusterBucket = new Map<string, CandidateArticle[]>();
  const evaluations: MIVEvaluation[] = [];
  const falseStandaloneList: any[] = [];

  // 1. Assign each article to its parent Knowledge Anchor or Mark as Standalone / Skip
  articles.forEach(art => {
    const text = `${art.title} ${art.text}`.toLowerCase();

    // Check if matching any Sovereign Knowledge Anchor
    ...
    if (matchedAnchor) {
      ...
      // Check for standalone statutory / AIFI paper / distinct sovereign notice
  ...
  // 2. Synthesize High-Density Editorial Memory Units from Cluster Buckets
  const synthesizedUnits: SynthesizedMemoryUnitV3[] = [];
  ...
  for (const [clusterId, arts] of clusterBucket.entries()) {
    ...
    // Construct Master Memory Unit
    const memoryUnit: SynthesizedMemoryUnitV3 = {
      unitId: clusterId,
      title,
      category,
      tier,
    ...
    // Record MIV evaluation for Master Article
    evaluations.push({
      artId: master.artId,
      page: master.page,
      title: master.title,
    ...
    // Record MIV evaluation for Attached Articles
